"""
level_meter.py - stand-alone bring-up test for the shield (no PC software).
...
Run it from the REPL with:  import level_meter
...
adc.begin(bits=BITS, gain_db=GAIN_DB)

i2s = I2S(0, sck=Pin(PIN_BCLK), ws=Pin(PIN_FSYNC), sd=Pin(PIN_SDOUT),
          mode=I2S.RX, bits=BITS, format=I2S.STEREO, rate=SAMPLE_RATE,
...
buf = array("i", (0 for _ in range(FRAMES * 2)))
mv = memoryview(buf)
